...
var compile = function(fileName, assets, S3, options, method, type, timestamp, callback) {
	...
	return function(err, results) {
		...
		var md5 = crypto.createHash('md5').update(results).digest("hex");
		var filePrecompile = [fileName.slice(0, position), '-' + md5, fileName.slice(position)].join('');
		checkFileModified(S3, filePrecompile, timestamp, function(exist) {
			if (exist && method) {
			...
			switch(method) {
				case 'uglify':
					...
					async.map([1], function(item, iter) {
						...
						tmp.replace(/(?!['"])CDN\(['"](.*?)['"]\)(?=['"])/ig, function(match, url) {
							...
							readUtf8(absUrl, compile(url, absUrl, S3, options, 'image', 'image/'+path.extname(url).substr(1), Date.now(), function(filePrecompile) {
								results = results.replace(match, options.domain + filePrecompile[url])
				...
				case 'minify':
					...
					for (var key in results) {
						...
						var fileBasePath  = path.dirname(path.join(options.publicDir, filePrecompile));
				...
				case 'font':
					...
					filePrecompile  = filePrecompile.split("?")[0].split("#")[0];
...
var checkFileModified = function(S3, fileName, timestamp,callback) {
...
var CDN = function(app, options, callback) {
	...
	return function(req, res) {
		return function(assets, attributes) {
			...
			return renderTag(options, resultsPrecompile[assets.substr(1)], attributes);
		};
	};
