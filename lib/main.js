...
var checkAndUploadS3 = function(data, fileName, S3, timestamp, headers, finishUpload) {
		...
		switch(method) {
			case 'uglify':
				...
				async.map([1], function(item, iter) {
					...
					checkAndUploadS3(final_code, fileName, S3, timestamp, headers, finishUpload);
				})
				break;
			case 'minify':
				...
				checkAndUploadS3(final_code.join('\n'), fileName, S3, timestamp, headers, finishUpload);
				break;
			case 'optipng':
				var img = assets;
				...
				optipng.on('exit', function(code) {
					...
					fs.readFile(img, function(err, data) {
						checkAndUploadS3(data, fileName, S3, timestamp, headers, finishUpload);
					});
				});
				break;
			case 'jpegtran':
